...
function readPianoTemplate(data, keywords) {
    ...
    var read_piani_html = `
        <div class="btn-group margin-bottom-1em" role="group" aria-label="...">
            ...
            <button id="piani-btn-group" type="button" class="btn btn-default active">Piani</button>
        ...
        <!-- form di ricerca piano -->
        <form id='search-piano-form' action='#' method='post'>
        <div class='input-group pull-left w-30-pct'>

            <input type='text' value='`+ keywords + `' name='keywords' class='form-control product-search-keywords' placeholder='Cerca piano...' />
        ...
        <div id='create-piano' class='btn btn-primary pull-right m-b-15px create-piano-button'>
        ...
        read_piani_html += `
            <tr>
                ...
                <td class='text-align-center'>
                    <!-- button di lettura del piano -->
                    <button class='btn btn-primary m-r-10px read-one-piano-button' data-id='` + val.id + `'>
                        <span class='glyphicon glyphicon-eye-open'></span> Leggi
                    </button>
            
                    <!-- button di modifica del piano -->
                    <button class='btn btn-info m-r-10px update-piano-button' data-id='` + val.id + `'>
                        <span class='glyphicon glyphicon-edit'></span> Modifica
                    </button>
            
                    <!-- button di eliminazione del piano -->
                    <button class='btn btn-danger delete-piano-button' data-id='` + val.id + `'>
    ...
    //fine tabella
    read_piani_html += `</table>`;

    $("#page-content").html(read_piani_html);
}